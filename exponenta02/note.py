...
from func import *  # choose_command
    ...
    def search_notes(self, search_text: str):
        ...
        if matching_notes:
            ...
            for index, note in matching_notes:
                print(
                    f"Note: {index}, Text: {note['text']}, Tags: {note['tags']}")
        else:
...
COMMANDS_DICT = {
    'add': {'note or/and #tags': None},
    'change': {'num note': None},
    'delete': {'num': None},
    'find': {'anything': None},
    'help': None,
    'show': {'10'},
...
def note_main():
    help()
    while True:
        user_input = choose_command(COMMANDS_DICT)
        if user_input.lower().startswith(("exit", "close", "quit")):
            notebook.save_notes()
            break
        parser(user_input)
